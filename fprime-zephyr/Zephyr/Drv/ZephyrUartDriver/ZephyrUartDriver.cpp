...
namespace Zephyr {
    ...
    void ZephyrUartDriver::configure(const struct device *dev) {
        ...
        )
    {
        ...
        U32 recv_size = ring_buf_get(&this->m_ring_buf, fwBuffer.getData(), fwBuffer.getSize());
        ...
        )
    {
        for (U32 i = 0; i < sendBuffer.getSize(); i++) {
            uart_poll_out(this->m_dev, sendBuffer[i]);
        }
